...
**Документ:** Документация, содержащая информацию, необходимую для эксплуатации экземпляра программного обеспечения, предоставленного для проведения экспертной проверки
...
## 2. Инструкция для экспертной проверки

## 2.0. Подготовка окружения

Распакуйте архив с ПО. Убедитесь, что в файле .env установлен параметр DEMO_MODE=True. В этом режиме ПО будет использовать локальные файлы-заглушки, что позволяет проверить интерфейс и логику работы без настройки API-ключей и доступа к внешним ресурсам.
